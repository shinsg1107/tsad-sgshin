...
def get_vus_metrics(score, labels, slidingWindow=100, version='opt', thre=250):
    """
    Returns VUS-ROC and VUS-PR only.
    
    Args:
        score: anomaly scores (np.ndarray)
        labels: ground-truth binary labels (np.ndarray)
        slidingWindow: sliding window size
        version: 'opt' or 'opt_mem'
        thre: number of thresholds
    
    Returns:
        dict with 'VUS-ROC' and 'VUS-PR'
    """
    ...
    for l in labels:
        if l == 1:
            ...
            if in_anomaly:
                ...
                length = 0  # ← 여기서 length 초기화가 else 블록 안에 있어서
                in_anomaly = False
    if in_anomaly:
        anomaly_lengths.append(length)
    
    print(f"anomaly segments: {len(anomaly_lengths)}")
    print(f"lengths: min={min(anomaly_lengths)}, median={np.median(anomaly_lengths)}, max={max(anomaly_lengths)}")
    
    return int(np.median(anomaly_lengths)) if anomaly_lengths else 100