...
class OracleAD_MHSA(nn.Module):
    def __init__(self, cfg):
        ...
        self.backend: str = getattr(self.cfg_m, "BACKEND", "torch")
        self.dim: int = int(self.cfg_m.DIM)
        self.num_heads: int = int(self.cfg_m.NUM_HEADS)
        self.bias: bool = bool(getattr(self.cfg_m, "BIAS", True))
        ...
        self.mask_self: bool = bool(getattr(self.cfg_m, "MASK_SELF", False))
        ...
        # Causal graph 로드
        # ------------------------------------------------------------------ #
        self.register_buffer("causal_bias", None)  # [N, N] buffer
        cfg_causal = getattr(self.cfg_m, "CAUSAL", None)

        if cfg_causal is not None and bool(getattr(cfg_causal, "ENABLE", False)):
            path = getattr(cfg_causal, "GRAPH_PATH", "")
            if path and os.path.isfile(path):
                # .pt 또는 .npy 지원
                if path.endswith('.pt'):
                    cg = torch.load(path, map_location='cpu').float()
                    cg = cg.numpy()
                elif path.endswith('.npy'):
                    cg = np.load(path).astype(np.float32)
                else:
                    raise ValueError(f"Unsupported causal graph format: {path}")

                assert cg.ndim == 2 and cg.shape[0] == cg.shape[1], \
                    f"Causal graph must be square [N,N], got {cg.shape}"

                # transpose: causal_graph[i,j] = i→j
                # attention A[i,j] = i가 j를 attend
                # j→i (j가 i에 영향) → i는 j를 attend해야 함
                # → transpose해서 A[i,j]에 cg[j,i] 반영
                if bool(getattr(cfg_causal, "TRANSPOSE", False)):
                    cg = cg.T

                # [0,1] 정규화
                if bool(getattr(cfg_causal, "NORMALIZE", True)):
                    cg_min, cg_max = cg.min(), cg.max()
                    if cg_max > cg_min:
                        cg = (cg - cg_min) / (cg_max - cg_min)

                self.causal_alpha = float(getattr(cfg_causal, "ALPHA", 1.0))
                self.causal_mode  = getattr(cfg_causal, "MODE", "additive")

                # buffer로 등록 (device 이동 자동)
                self.register_buffer(
                    "causal_bias",
                    torch.from_numpy(cg)
                )
                print(f"[MHSA] Causal graph loaded: {path}")
                print(f"  shape={cg.shape}, mode={self.causal_mode}, "
                      f"alpha={self.causal_alpha}, "
                      f"mean={cg.mean():.4f}, max={cg.max():.4f}")
            else:
                print(f"[MHSA] CAUSAL.ENABLE=True but path not found: {path}")
        ...
        if self.backend == "torch":
            ...
            raise ValueError(f"Unknown MHSA BACKEND: {self.backend}")

    def _apply_causal_bias(self, A: torch.Tensor) -> torch.Tensor:
        """
        A: [B, N, N] pre-softmax attention scores
        causal_bias: [N, N]
        """
        if self.causal_bias is None:
            return A

        cg = self.causal_bias.unsqueeze(0)  # [1, N, N]

        if self.causal_mode == "additive":
            # causal edge가 있는 쌍의 attention score를 높임
            return A + self.causal_alpha * cg

        elif self.causal_mode == "multiplicative":
            # causal edge 없는 쌍은 억제, 있는 쌍은 강화
            return A * (1.0 + self.causal_alpha * cg)

        return A

    def _get_attn_mask(self, N: int, device, dtype) -> torch.Tensor:
        if (self._cached_mask is None) or (self._cached_N != N) or \
    ...
    def forward(self, C: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.backend == "torch":
            ...
            # causal bias → torch backend는 attn_mask로 전달
            if self.causal_bias is not None:
                causal_mask = self._apply_causal_bias(
                    torch.zeros(N, N, device=C.device, dtype=C.dtype)
                ).squeeze(0)  # [N, N]
                attn_mask = causal_mask if attn_mask is None \
                            else attn_mask + causal_mask
            ...
            if attn_w.dim() == 3:
                attn_w = attn_w.unsqueeze(1).repeat(1, self.num_heads, 1, 1)
            out = attn_out
            ...
            for h in range(self.num_heads):
                ...
                # causal bias 적용 (softmax 전)
                A = self._apply_causal_bias(A)
